...
void Playbook_DoNothing::configure(int numPlayers) {
    for(int i = 0; i < numPlayers; i++) {
        ...
        Role_SecondAttacker *rl_secondAttacker = new Role_SecondAttacker();
        usesRole(rl_secondAttacker);
        _rl_secondAttacker.push_back(rl_secondAttacker);
    }
}
...
void Playbook_DoNothing::run(int numPlayers) {
    ...
    setPlayerRole(1, _rl_secondAttacker.at(1));
}
